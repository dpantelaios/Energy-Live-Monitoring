...
const app = express();
const backend = require('./display_backend_FF');
const PORT = 5000;
...
//data used for display_ff database connection
let dbConfig = {
...
app.get("/getData", (req, res) => {
    ...
    let requested_country_from = req.query.country_from;
    let requested_country_to = req.query.country_to;
    let sql = `SELECT flowValue, dateTime, resolutionCode FROM flowvalue WHERE \`dateTime\` >= \'${requested_date}\' and inMapCode = \'${requested_country_to}\' and outMapCode = \'${requested_country_from}\' ORDER BY \`dateTime\` ASC`;
    console.log(sql);
...
let token = " ";
app.get("/", (req, res) => {
    res.render( require('path').resolve(__dirname) + "/display_FF.html", { token: token}); //redirect to display physicals flows page
});

//get parameter in order to know if new data came
app.get("/getPlotBoolean", (req, res) => {
    const {plot_parameter} = require('./display_backend_FF.js'); //read the signal to re-plot from backend module
